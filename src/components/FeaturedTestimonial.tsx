...
const featured = [
  {
    quote: (
      <>
        ...
        a true expert in his field but <span className="text-red-ribbon">genuinely cares</span> about your growth.&rdquo;
      </>
    ),
    name: "Bernadette L.",
    role: "Mental Coach",
  ...
  },
  {
    quote: (
      <>
        &ldquo;Working with Danny has been a <span className="text-red-ribbon">game-changer</span> for both me and my
  ...
  },
  {
    quote: (
      <>
        ...
        challenged me and helped me <span className="text-red-ribbon">find clarity</span>.&rdquo;
      </>
    ),
    name: "Dasha B.",
...
export function FeaturedTestimonial() {
  ...
  return (
    <section
      className="border-t border-mine-shaft/50 px-4 py-24 md:px-[15px]"
      style={{
        background: "linear-gradient(180deg, rgba(13, 13, 13, 1) 0%, rgba(38, 38, 38, 0.2) 100%)",
      }}
    >
      <div className="mx-auto grid max-w-6xl gap-12 lg:grid-cols-2">
        <div>
          <Quote className="mb-6 h-6 w-6 text-red-ribbon" />
              ...
              >
                ...
                <div className="mt-8 flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-base text-oslo-gray leading-6">{item.role}</p>
                  </div>
                </div>
              </div>
            ))}
            ...
            {featured.map((_, index) => (
              <button
                ...
                className={`h-2 rounded-full transition-all ${
                  active === index ? "w-6 bg-red-ribbon" : "w-6 bg-white/20"
                }`}
              />
            ))}
          </div>
        ...
        <div className="relative min-h-[368px]">
          {cards.map((card, index) => (
            <button
              ...
              className="absolute rounded-3xl border border-white/10 bg-transparent p-6 text-left transition-all duration-300 hover:-translate-y-1 hover:border-red-ribbon/50"
              style={{
                top: index * 196,
                left: index * 32,
                width: `calc(100% - ${index * 32}px)`,
                zIndex: active === index + 1 ? 30 : 20 - index,
                background: "linear-gradient(185deg, rgba(255, 255, 255, 0.1) 0%, rgba(255, 255, 255, 0) 100%)",
              }}
            >
              <div className="mb-4 flex gap-1">
                {Array.from({ length: 5 }).map((_, i) => (
                  <Star key={i} className="h-4 w-4 fill-whiskey text-whiskey" />
                ))}
              </div>
              <p className="text-lg leading-7 text-oslo-gray">{card.quote}</p>
              <p className="mt-4 text-sm font-medium text-oslo-gray/70">{card.author}</p>
            </button>
          ))}
        </div>
      </div>
