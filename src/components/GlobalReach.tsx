...
export function GlobalReach() {
  return (
    <section
      ...
      style={{
        background:
          "linear-gradient(175deg, rgba(255, 255, 255, 0.1) 0%, rgba(255, 255, 255, 0) 50%, rgba(255, 255, 255, 0.1) 100%)",
      ...
      <div className="spotlight-content grid items-center gap-10 lg:grid-cols-2">
        <div className="space-y-4">
          <div className="flex items-center gap-3">
            <span className="text-sm font-medium uppercase tracking-[0.1em] text-red-ribbon">03</span>
            <span className="text-sm font-medium text-oslo-gray">Global Reach</span>
          </div>
          <h2 className="text-5xl font-medium leading-[96px] tracking-[-0.05em] md:text-6xl md:leading-[96px] whitespace-nowrap">
            Speaking <span className="font-playfair italic text-oslo-gray">Worldwide</span>
          </h2>
          <p className="pb-4 text-lg leading-7 text-oslo-gray">
            From Singapore to Scandinavia, delivering AI keynotes at major conferences and corporate events across the
            globe.
          </p>
          <div className="grid grid-cols-2 gap-4">
            {stats.map((stat) => (
              <div key={stat} className="inline-flex items-center gap-2 rounded-lg border border-white/10 bg-white/5 px-4 py-2">
                <span className="h-2 w-2 rounded-full bg-red-ribbon/50" />
                <span className="text-sm font-medium text-oslo-gray">{stat}</span>
              </div>
            ))}
          </div>
        </div>
            ...
            <div className="relative h-full w-full rotate-clockwise">
              {flags.map((flag, index) => {
                ...
                return (
                  ...
                  >
                    <div className="rotate-counter flex h-full w-full items-center justify-center rounded-full border-2 border-red-ribbon/50 bg-[#262626] text-3xl shadow-lg">
                      {flag}
                    </div>
                  </div>
                );
