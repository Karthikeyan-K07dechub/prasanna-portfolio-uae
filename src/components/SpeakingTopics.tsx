      ...
      <div className="divide-y divide-black/15 border-y border-black/15">
        {areas.map((area, index) => (
          <article key={t(area.title)} className="grid gap-4 py-7 text-center sm:py-8 sm:text-start lg:grid-cols-[0.9fr_1.1fr] lg:gap-12">
            ...
            <ul aria-label={`${t(area.title)} — ${t("services")}`} className="mx-auto grid w-fit max-w-full gap-x-6 gap-y-3 text-start sm:mx-0 sm:w-auto sm:grid-cols-2 lg:pt-1">
              {area.services.map((service) => (
                <li key={t(service)} className="flex items-start gap-3 text-base leading-7">
