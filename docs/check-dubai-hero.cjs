  ...
  try {
    ...
    for (const [width,height] of [[390,844],[768,1024],[1440,900]]) {
      ...
      for (const section of (['hero'])) {
        ...
        fs.writeFileSync(path.join('docs', 'dubai-hero-' + label + '-' + section + '.png'), Buffer.from(shot.data, 'base64'));
      }
    }
    fs.writeFileSync('docs/dubai-hero-'+(process.argv[2]||'audit')+'.json',JSON.stringify(report,null,2));
