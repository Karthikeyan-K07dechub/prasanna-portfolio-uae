...
(async () => {
  ...
  try {
    ...
    await sleep(3000);
    const report=[];
    for (const [width,height] of [[320,568],[360,800],[390,844],[430,932],[568,320],[640,960],[768,1024],[820,1180],[1024,768],[1280,800],[1440,900],[1920,1080],[2560,1440]].filter(([w]) => process.argv[2] !== "visual" || [320,768,1024].includes(w))) {
      ...
      for (const section of (process.argv[2] !== 'before' && [320,768,1024].includes(width) ? ['hero','insights','contact'] : [])) {
        ...
        fs.writeFileSync(path.join('docs', 'responsive-' + label + '-' + section + '.png'), Buffer.from(shot.data, 'base64'));
      }
    }
    fs.writeFileSync('docs/responsive-'+(process.argv[2]||'audit')+'.json',JSON.stringify(report,null,2));
